...
No error message appears, and the test exits under the expected conditions (after 20 steps). This is a successful (if short) test. The following graphic contains all the states and transitions used in this section and the next one, as an aid to assist with tracing these test examples.
...
The graphics used in the previous sections were generated through modifications of this example code, as was the following graphic capturing a subset of the state model inferred from the \texttt{monica-tests-yeagerized} repository. In this graphic, edge labels have been turned off for legibility, and aspect ratio has been modified.
