...
Consider Monica's pages, which are already built into our test suite, to be states.
...
For the purpose of our discussions, these pages will constitute the entire set of states in the system under test. Conveniently, each of them is represented as a Python class.
...
Note that Python \texttt{None} constant is used as a reference to the uninitialized system. Yeager treats \texttt{None} as a special node in the implied state model our annotations provide: it's assumed to be the entry point.
...
No error message, an exit under our expected conditions (after 20 steps), and a healthy system.
